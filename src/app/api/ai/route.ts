...
const buildPrompt = (userQuestion: string, conversationHistory: ChatMessage[]) => {
  const historyText = conversationHistory
    .filter((msg) => typeof msg.content === 'string' && (msg.role === 'user' || msg.role === 'ai'))
...
export async function POST(request: Request) {
  try {
    const { userQuestion, conversationHistory = [] } = await request.json();
    ...
    const response = await ai.models.generateContent({
      ...
      contents: buildPrompt(userQuestion.trim(), Array.isArray(conversationHistory) ? conversationHistory : []),
      config: {
    ...
    return NextResponse.json({
      response: response.text || "I apologize, I couldn't generate a response at this moment.",
    });
  } catch (error) {
    console.error('AI Service Error:', error);
    return NextResponse.json(
      { error: 'Our AI assistant is currently unavailable. Please try again later or contact us directly.' },
      { status: 500 }
    );
  }
}
